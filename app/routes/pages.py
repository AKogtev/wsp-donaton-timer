...
from ..core.db import get_setting, save_tokens

router = APIRouter()

# Папка шаблонов — абсолютный путь от файла
...
async def config_page(request: Request):
    ...
    return templates.TemplateResponse("config.html", {"request": request, "token": token, "coef": state.rub_to_sec})

@router.get("/auth")
async def auth_page(request: Request):
    return templates.TemplateResponse("auth.html", {"request": request, "redirect_uri": state.oauth_redirect_uri})

@router.post("/start_auth")
async def start_auth(request: Request, client_id: str = Form(...), client_secret: str = Form(...)):
    ...
    auth_url = "https://www.donationalerts.com/oauth/authorize?" + urlencode(params, quote_via=quote_plus)
