...
async def timer_cfg_ws(websocket: WebSocket):
    """Передаём настройки таймера (сейчас — только цвет текста)."""
    await websocket.accept()
    state.timer_cfg_clients.append(websocket)
    # отсылаем текущий цвет при подключении
    await websocket.send_text(state.timer_text_color)
    try:
        while True:
            await websocket.receive_text()  # ничего не ждём от клиента
    except WebSocketDisconnect:
        ...
        while True:
            ...
            if cmd.startswith("set "):
                try:
                    ...
                    await websocket.send_text(f"Установлено время: {format_time(state.remaining_seconds)}")
                ...
                if not (state.oauth_client_id and state.oauth_client_secret and (get_setting('access_token') or state.oauth_access_token)):
                    await websocket.send_text("Не заданы client_id/secret или пустой Access Token")
                    continue
                ...
                await websocket.send_text("Access Token сохранён, пробуем подключиться к DonationAlerts...")

            elif cmd.startswith("coef "):
                try:
                    val = float(cmd[5:].strip().replace(",", "."))  # поддержим запятую как разделитель
                    if val < 0:
                        await websocket.send_text("Ошибка: коэффициент не может быть отрицательным")
                    else:
                        state.rub_to_sec = val
                        set_setting("rub_to_sec", str(val))
                        # g-формат убирает лишние нули: 4.5 → "4.5", 10.0 → "10"
                        await websocket.send_text(f"Соотношение изменено: 1₽ = {val:g} секунд")
                except Exception:
                ...
                if col not in ("black", "white"):
                    await websocket.send_text("Ошибка: допустимы только 'black' или 'white'")
                else:
